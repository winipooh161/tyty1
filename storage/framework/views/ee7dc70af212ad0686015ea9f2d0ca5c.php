...
<div class="editor-container">
    ...
    <div id="coverPreviewContainer" class="cover-container mb-3">
        <?php if(isset($userTemplate) && $userTemplate && $userTemplate->cover_path): ?>
            <?php
            ...
            <?php elseif($userTemplate->cover_type === 'image' && $coverExists): ?>
                <img src="<?php echo e(asset($coverPath)); ?>" class="cover-image" alt="<?php echo e($userTemplate->name ?? 'Обложка шаблона'); ?>">
            <?php else: ?>
                <div class="cover-fallback">
                    <div class="fallback-content">
                        <i class="bi bi-image text-white mb-2" style="font-size: 3rem;"></i>
                        <h3 class="text-white"><?php echo e($userTemplate->name ?? 'Предпросмотр обложки'); ?></h3>
                    </div>
                </div>
            <?php endif; ?>
        <?php elseif(session('media_editor_file')): ?>
            <?php
            ...
            <?php if($mediaType === 'video' && $coverExists): ?>
                <video id="coverVideo" class="cover-video" autoplay loop muted playsinline>
                    <source src="<?php echo e(asset($coverPath)); ?>" type="video/<?php echo e(pathinfo($mediaFile, PATHINFO_EXTENSION)); ?>">
                    Ваш браузер не поддерживает видео.
                </video>
            <?php elseif($mediaType === 'image' && $coverExists): ?>
                <img src="<?php echo e(asset($coverPath)); ?>" class="cover-image" alt="Обложка шаблона">
            <?php else: ?>
                <div class="cover-fallback">
                    <div class="fallback-content">
    ...
    <div id="template-preview" class="template-container fullscreen">
        <?php echo isset($userTemplate) ? $userTemplate->html_content : $template->html_content; ?>
    ...
    <form id="template-save-form" action="<?php echo e(route('client.templates.save', $template->id)); ?>" method="POST" enctype="multipart/form-data" style="display: none;">
        <?php echo csrf_field(); ?>
        <input type="hidden" id="template-name" name="name" value="<?php echo e($userTemplate->name ?? $template->name); ?>">
        ...
        <input type="hidden" name="media_editor_file" value="<?php echo e(session('media_editor_file') ?? ''); ?>">
        <input type="hidden" name="media_editor_type" value="<?php echo e(session('media_editor_type') ?? ''); ?>">
        <?php if(isset($userTemplate) && $userTemplate->cover_path): ?>
