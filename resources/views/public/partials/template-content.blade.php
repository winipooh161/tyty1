    ...
    <div class="template-actions-container">
        @auth
            ...
            @if(!$alreadyAcquired && !$isOwner && $isAvailable)
                ...
                <form id="acquireTemplateForm" action="{{ route('series.acquire', $userTemplate->id) }}" method="POST" class="mt-4" onsubmit="handleFormSubmit(this, event)">
                    @csrf
                    <!-- Отладочные поля -->
        ...
        @else
            <div class="alert alert-info mt-4">
                <i class="bi bi-info-circle me-2"></i> Для получения шаблона необходимо <a href="{{ route('login') }}">войти</a> или <a href="{{ route('register') }}">зарегистрироваться</a>.
            </div>
        @endauth
    </div>
</div>
