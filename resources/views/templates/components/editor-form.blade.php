...
<form id="template-save-form" 
      action="{{ isset($userTemplate) ? route('templates.save', $userTemplate->template_id) : route('templates.save', $template->id) }}" 
      method="POST" 
    ...
    <input type="hidden" id="custom_data" name="custom_data" value="{{ $userTemplate->custom_data ?? $template->custom_data ?? '{}' }}">
    
    <!-- Если это новый шаблон, добавляем флаг -->
    @if(isset($is_new_template) && $is_new_template)
    ...
    @if(session('media_editor_file'))
        <input type="hidden" name="media_file" value="{{ session('media_editor_file') }}">
        <input type="hidden" name="media_type" value="{{ session('media_editor_type') }}">
    @endif
...
<div class="loading-spinner-overlay" id="form-submit-spinner">
    <div class="loading-spinner-container">
        <div class="loading-spinner">
            <img src="{{ asset('images/center-icon.svg') }}" class="loading-spinner-icon" alt="Loading...">
        </div>
