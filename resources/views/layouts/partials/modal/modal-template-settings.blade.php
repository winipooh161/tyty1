            ...
            <div class="modal-panel-body">
                <form id="template-form" action="{{ route('client.templates.save', $template->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="name" name="name" value="{{ $userTemplate->name ?? $template->name }}" required>
                    </div>
                    ...
                    <div class="mb-4">
                        <div class="mb-3">
                            ...
                            <div class="file-status mb-2" id="fileStatusIndicator">
                                @if(isset($media_editor_file) || (isset($userTemplate) && $userTemplate->cover_path))
                            ...
                            <div class="file-preview-container mb-3" id="filePreviewContainer" 
                                 style="display: {{ isset($media_editor_file) || (isset($userTemplate) && $userTemplate->cover_path) ? 'block' : 'none' }}">
                                <div class="card">
                                    <div class="card-body">
                                        <h6 class="text-center mb-2">Выбранный файл</h6>
                                        <div class="preview-content text-center">
                                            @if(isset($media_editor_file))
                                                @if($media_editor_type == 'image')
                                                    <img src="{{ asset('storage/template_covers/' . $media_editor_file) }}" class="img-fluid preview-img" alt="Preview">
                                                @else
                                                    <video src="{{ asset('storage/template_covers/' . $media_editor_file) }}" class="img-fluid preview-video" controls></video>
                                                @endif
                                                <div class="mt-1 text-muted">Файл из редактора медиа</div>
                                            @elseif(isset($userTemplate) && $userTemplate->cover_path)
                                                @if($userTemplate->cover_type == 'image')
                                                    <img src="{{ asset('storage/template_covers/' . $userTemplate->cover_path) }}" class="img-fluid preview-img" alt="Preview">
                                                @else
                                                    <video src="{{ asset('storage/template_covers/' . $userTemplate->cover_path) }}" class="img-fluid preview-video" controls></video>
                                                @endif
                            ...
                            <div class="input-group">
                                <input type="file" class="form-control" id="cover_file" name="cover_file" 
                                       ...
                                       {{ !isset($media_editor_file) && (!isset($userTemplate) || !$userTemplate->cover_path) ? 'required' : '' }}>
                            ...
                            @if(isset($media_editor_file))
                                <input type="hidden" name="media_editor_file" id="media_editor_file" value="{{ $media_editor_file }}">
                                <input type="hidden" name="media_editor_type" id="media_editor_type" value="{{ $media_editor_type }}">
                        ...
                        <input type="hidden" id="html_content" name="html_content" value="{{ $userTemplate->html_content ?? $template->html_content }}">
                        ...
                        <div class="card mb-3">
                            ...
                            <div class="card-body series-settings" style="display: {{ (isset($userTemplate) && isset($userTemplate->custom_data['is_series']) && $userTemplate->custom_data['is_series']) ? 'block' : 'none' }}">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control" id="series_quantity" min="1" max="1000" 
                                        value="{{ isset($userTemplate) && isset($userTemplate->custom_data['series_quantity']) ? $userTemplate->custom_data['series_quantity'] : 1 }}">
                                ...
                                <div class="mb-2">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="limit_one_per_user" 
                                            {{ (isset($userTemplate) && isset($userTemplate->custom_data['limit_one_per_user']) && $userTemplate->custom_data['limit_one_per_user']) ? 'checked' : '' }}>
                        ...
                        @if(isset($vipUsers) && count($vipUsers) > 0)
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="target_user_id" name="target_user_id">
                                ...
                                @foreach($vipUsers as $vipUser)
                                    <option value="{{ $vipUser->id }}" {{ (isset($userTemplate) && $userTemplate->target_user_id == $vipUser->id) ? 'selected' : '' }}>
                                        {{ $vipUser->name }}
                                    </option>
                                @endforeach
                            </select>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (isNewTemplateInput) {
        isNewTemplateInput.setAttribute('data-template-id', '{{ $template->id ?? 0 }}');
    }
