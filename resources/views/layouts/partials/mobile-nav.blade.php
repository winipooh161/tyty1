<nav class="mb-navigation mb-dock hide-desktop">
    <div class="mb-fixed-container">
        <div class="mb-scroller" id="nav-scroll-container">
            <div class="mb-icons-container" id="nav-icons-container">
                {{--    
          <div class="mb-icon-wrapper" data-icon-id="email" data-original="true">
            <a class="mb-nav-link {{ request()->routeIs('home') ? 'mb-active' : '' }}" href="{{ route('home') }}" onclick="centerNavItem(this.parentElement)">
              <div class="mb-nav-icon-wrap">
                <img class="mb-nav-icon" src="{{ asset('images/icons/email.svg') }}" alt="Почта">
              </div>
            </a>
          </div> --}}
                    ...
                    <div class="mb-icon-wrapper" data-icon-id="qr-scanner" data-modal="true"
                        ...
                        <a class="mb-nav-link no-spinner" href="javascript:void(0);"
                            ...
                            <div class="mb-nav-icon-wrap">
                                <img class="mb-nav-icon" src="{{ asset('images/icons/scan.svg') }}" alt="QR сканер">
                            </div>
                        </a>
                    </div>
                    ...
                    <div class="mb-icon-wrapper mb-center-wrapper" data-icon-id="center" data-original="true">
                        <a class="mb-nav-link mb-center-link {{ request()->routeIs('home') ? 'mb-active' : '' }}"
                            href="{{ route('home') }}" onclick="centerNavItem(this.parentElement)">
                            <div class="mb-nav-icon-wrap mb-center-icon-wrap">
                                <img class="mb-nav-icon mb-center-nav-icon" src="{{ asset('images/center-icon.svg') }}"
                                    alt="Центр">
                            </div>
                        </a>
                    </div>
                    ...
                    @if (Auth::user()->role === 'client' || Auth::user()->role === 'admin')
                        <div class="mb-icon-wrapper" data-icon-id="profile" data-original="true">
                            <a class="mb-nav-link {{ request()->routeIs('user.templates*') ? 'mb-active' : '' }}"
                                href="{{ route('user.templates') }}" onclick="centerNavItem(this.parentElement)">
                                <div class="mb-nav-icon-wrap">
                                    <img class="mb-nav-icon" src="{{ asset('images/icons/person.svg') }}"
                                        alt="Профиль">
                                </div>
                            </a>
                        </div>
 ...
 <div class="mb-icon-wrapper" data-icon-id="create" data-original="true">
                    <a class="mb-nav-link {{ request()->routeIs('media.editor') || request()->routeIs('create.template') ? 'mb-active' : '' }}"
                        href="{{ route('create.template') }}" onclick="centerNavItem(this.parentElement)">
                        <div class="mb-nav-icon-wrap">
                            <img class="mb-nav-icon" src="{{ asset('images/icons/plus-1.svg') }}" alt="Создать">
                        </div>
                    </a>
                </div>
                    @if (Auth::user()->role === 'admin')
                        <div class="mb-icon-wrapper" data-icon-id="admin" data-original="true">
                            <a class="mb-nav-link {{ request()->routeIs('admin.dashboard') ? 'mb-active' : '' }}"
                                href="{{ route('admin.dashboard') }}" onclick="centerNavItem(this.parentElement)">
                                <div class="mb-nav-icon-wrap">
                                    <img class="mb-nav-icon" src="{{ asset('images/icons/speedometer.svg') }}"
                                        alt="Админ панель">
                                </div>
                            </a>
                        </div>
